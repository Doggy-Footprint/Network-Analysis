...
| M2·M5·M7: 실제 개선 검증 | [개선 효과 검증](ANALYSIS_APPROACH.md#개선-효과-검증) |

## 구현 상태와 재정렬

현재 구현 기준선은 Python snapshot 분석, agent-view 그래프, 고정 harness profile 재생, 정적 병목 후보, 관측 trace 비교다. 이는 구현된 범위를 뜻하며 M2의 실제 harness 검증과 M3 후보의 실제 에이전트 대조는 아직 수행되지 않았다. 지원 범위와 향후 실험 기준은 [Harness profile support and evaluation constraints](research/4f38a2c1d760be95-harness-evaluation-contract.md)에 기록한다.
...
구현 상태: 고정 profile, trace 스키마, snapshot/profile 결합 검증과 출력 재생 비교를 구현했다. 실제 팀 harness 기준선 확인과 독립 전후 실험은 미수행이다.
...
구현 상태: `bottlenecks.v1` JSON, 해당 JSON의 HTML 변환, Python 정적 후보 생성을 구현했다. 실제 에이전트 관측을 통한 후보 검증은 미수행이다.
