...
기존 `agent_view/`, `profiles/agent_view.v3.yaml`, `agent-view.json`, `report/m1/`이 기반이다. 완료는 이전 표현·직렬화 계약에 한정한다.
...
구현 상태: 고정 profile, trace 스키마, snapshot/profile 결합 검증을 구현했다. Profile 지원표, 독립 평가 사례, 실행 envelope, 확인·결과 충족 판정, 단일 요인 비교와 결정적 CLI를 구현했다. 실제 팀 harness 기준선 확인과 독립 전후 실험은 M8에서 수행한다.
