...
class SeedQueryTests(unittest.TestCase):
    def setUp(self):
        self.path = ROOT / "fixtures" / "seed_queries.v1.json"
        self.generator = CachedSeedQueryGenerator.from_path(self.path)

    def test_cached_lookup_returns_recorded_terms_and_generator_slots(self):
        task = "Change how long an issued access token stays valid."
        result = self.generator.generate(task)

        self.assertEqual(result.source, "cache")
        self.assertEqual(result.terms[0], SeedTerm("login", "content"))
        self.assertEqual(result.generator["model_id"], "hand-written")
        self.assertEqual(result.generator["backend"], "none")
        self.assertEqual(
            result.generator["fixture_sha256"],
            hashlib.sha256(self.path.read_bytes()).hexdigest(),
        )
        self.assertEqual(
            result.generator["raw_output"],
            "content:login\ncontent:token\ncontent:expire\ncontent:token_expiry",
        )

    def test_raw_output_is_the_recorded_string_not_a_rebuild_of_the_terms(self):
        fixture = json.loads(self.path.read_text(encoding="utf-8"))
        entry = {
            "task": "a task whose recording is deliberately not the joined terms",
            "raw_output": "I would start by grepping for the token helper.\ncontent:token",
            "terms": [{"term": "token", "surface": "content"}],
        }
        entry["task_sha256"] = hashlib.sha256(entry["task"].encode("utf-8")).hexdigest()
        fixture["entries"].append(entry)

        result = CachedSeedQueryGenerator(fixture, "c" * 64).generate(entry["task"])

        self.assertEqual(result.terms, (SeedTerm("token", "content"),))
        self.assertEqual(result.generator["raw_output"], entry["raw_output"])
        self.assertNotEqual(
            result.generator["raw_output"],
            "\n".join(f"{item.surface}:{item.term}" for item in result.terms),
        )

    def test_missing_task_raises_naming_the_task_and_hash(self):
        task = "a task that was never recorded"
        with self.assertRaises(SeedQueryError) as raised:
            self.generator.generate(task)

        message = str(raised.exception)
        self.assertIn(task, message)
        self.assertIn(hashlib.sha256(task.encode("utf-8")).hexdigest(), message)

    def test_entry_with_wrong_task_hash_raises(self):
        fixture = json.loads(self.path.read_text(encoding="utf-8"))
        fixture["entries"][0]["task_sha256"] = "0" * 64
        with self.assertRaises(SeedQueryError):
            CachedSeedQueryGenerator(fixture, "0" * 64)

    def test_unknown_surface_raises(self):
        fixture = json.loads(self.path.read_text(encoding="utf-8"))
        fixture["entries"][0]["terms"][0]["surface"] = "symbol"
        generator = CachedSeedQueryGenerator(fixture, "0" * 64)
        with self.assertRaises(SeedQueryError):
            generator.generate(fixture["entries"][0]["task"])

    def test_wrong_schema_raises(self):
        fixture = json.loads(self.path.read_text(encoding="utf-8"))
        fixture["schema"] = "seed_queries.v2"
        with self.assertRaises(SeedQueryError):
            CachedSeedQueryGenerator(fixture, "0" * 64)

    def test_explicit_seed_queries_win_over_the_cache(self):
        scenario = Scenario("s", "unrecorded task", ("n:a1",), (SeedTerm("alpha", "content"),))

        resolved, seed_set = resolve_seed_queries(scenario, self.generator)

        self.assertEqual(seed_set.source, "explicit")
        self.assertEqual(resolved.seed_terms, (SeedTerm("alpha", "content"),))

    def test_scenario_without_explicit_terms_uses_the_cache(self):
        task = "Change how long an issued access token stays valid."
        scenario = Scenario("s", task, ("n:a1",), ())

        resolved, seed_set = resolve_seed_queries(scenario, self.generator)

        self.assertEqual(seed_set.source, "cache")
        self.assertEqual(resolved.seed_terms, seed_set.terms)

    def test_explicit_generator_records_its_terms(self):
        result = ExplicitSeedQueries((SeedTerm("alpha", "content"),)).generate("task")

        self.assertEqual(result.generator["raw_output"], "content:alpha")
